...
<style>
    ...
    .kb-panel-body[hidden] { display: none; }
    ...
    .kb-title { margin: 0; color: var(--kb-deep); font-size: 15px; font-weight: 600; }
    ...
    .kb-orang { display: flex; align-items: center; gap: 10px; }
    .kb-avatar {
        display: grid;
        place-items: center;
        width: 34px;
    ...
    .kb-orang__nama { font-weight: 600; }
    .kb-orang__sub { display: block; margin-top: 2px; font-size: 10px; color: #8a8a7a; font-weight: 500; }
    ...
    .kb-badge--nonaktif { background: #fbe1de; color: #b3382c; }
    ...
    .kb-hint { margin: 6px 0 0; color: #6b6b60; font-size: 11px; }
    ...
    @media (max-width: 520px) {
        ...
        .kb-tabs { gap: 14px; overflow-x: auto; }
    }
</style>
...
<div class="kb">
    ...
    <section class="kb-card kb-panel">
        ...
        <div class="kb-tabs" role="tablist">
            <button type="button" class="kb-tab {{ $tabAktif === 'daftar' ? 'kb-tab--on' : '' }}" data-tab="daftar" role="tab">Daftar Mandor</button>
            <button type="button" class="kb-tab {{ $tabAktif === 'akun' ? 'kb-tab--on' : '' }}" data-tab="akun" role="tab">Kelola Akun</button>
            <button type="button" class="kb-tab {{ $tabAktif === 'jadwal' ? 'kb-tab--on' : '' }}" data-tab="jadwal" role="tab">Jadwal Mandor</button>
        </div>
        ...
        <div class="kb-panel-body" data-tab-panel="daftar" @if($tabAktif !== 'daftar') hidden @endif>
            <div class="kb-head">
                ...
                <div class="kb-toolbar">
                    <form method="get" action="{{ url('admin/mandor') }}" class="kb-toolbar" role="search">
                        ...
                        <select name="blok" class="kb-select" aria-label="Filter blok" onchange="this.form.submit()">
                            <option value="">Semua Blok</option>
                            @foreach ($bloks as $b)
                                <option value="{{ $b->nama_blok }}" {{ request('blok') === $b->nama_blok ? 'selected' : '' }}>{{ $b->nama_blok }}</option>
                            @endforeach
                        </select>

                        <select name="status" class="kb-select" aria-label="Filter status" onchange="this.form.submit()">
            ...
            <div class="kb-wrap">
                    ...
                    <table class="kb-table">
                        ...
                        <tbody>
                            @forelse ($mandors as $mandor)
                                <tr data-mandor="{{ json_encode([
                                    'id'          => $mandor->id,
                                    'nama'        => $mandor->nama,
                                    'phone'       => $mandor->phone,
                                    'afdeling'    => $mandor->afdeling,
                                    'blok_kelola' => $mandor->blok_kelola,
                                    'status'      => $mandor->status,
                                ]) }}">
                                    <td>{{ $mandors->firstItem() + $loop->index }}</td>
                                    <td>
                                        <span class="kb-orang">
                                            <span class="kb-avatar" aria-hidden="true">{{ strtoupper(substr($mandor->nama, 0, 1)) }}</span>
                                            <span>
                                    ...
                                    <td>{{ $mandor->phone }}</td>
                                    <td>{{ $mandor->jumlah_pekerja ?? 0 }} Orang</td>
                                    <td class="kb-c">
                                        <span class="kb-badge kb-badge--{{ $mandor->status }}">{{ ucfirst($mandor->status) }}</span>
                                    </td>
                                    <td class="kb-c">
                                        <span class="kb-actions">
                                            <button type="button" class="kb-icon" data-aksi="lihat-mandor" aria-label="Lihat {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>
                                                </svg>
                                            </button>
                                            <button type="button" class="kb-icon" data-aksi="ubah-mandor" aria-label="Ubah {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/>
                                                </svg>
                                            </button>
                                            <button type="button" class="kb-icon kb-icon--del" data-aksi="hapus-mandor" aria-label="Hapus {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                ...
                <div class="kb-foot">
                    <span>
                        @if ($mandors->total() > 0)
                            Menampilkan {{ $mandors->firstItem() }} - {{ $mandors->lastItem() }} dari {{ $mandors->total() }} data
                        @else
                            Menampilkan 0 data
                        @endif
                    </span>
                        ...
                        <tbody>
                            @forelse ($mandors as $mandor)
                                <tr data-akun="{{ json_encode([
                                    ...
                                    <td class="kb-c">
                                        <span class="kb-actions">
                                            <button type="button" class="kb-icon" data-aksi="lihat-akun" aria-label="Lihat akun {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/>
                                                </svg>
                                            </button>
                                            <button type="button" class="kb-icon" data-aksi="ubah-akun" aria-label="Ubah akun {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/>
                                                </svg>
                                            </button>
                                            <button type="button" class="kb-icon kb-icon--del" data-aksi="hapus-akun" aria-label="Hapus akun {{ $mandor->nama }}">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                                                </svg>
                                            </button>
                            ...
                            @empty
                                <tr>
                                    <td colspan="5" class="kb-empty">Belum ada akun mandor. Tambahkan mandor lebih dulu di tab "Daftar Mandor".</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            @forelse ($jadwals as $jadwal)
                                <tr data-jadwal="{{ json_encode([
                                    ...
                                    <td>{{ optional($jadwal->blok)->nama_blok }}</td>
                                    <td>{{ optional($jadwal->blok)->afdeling }}</td>
                                    <td>
                                    ...
                                    <td class="kb-c">
                                        <span class="kb-actions">
                                            <button type="button" class="kb-icon" data-aksi="ubah-jadwal" aria-label="Ubah jadwal">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/><path d="m15 5 4 4"/>
                                                </svg>
                                            </button>
                                            <button type="button" class="kb-icon kb-icon--del" data-aksi="hapus-jadwal" aria-label="Hapus jadwal">
                                                <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                                                    <path d="M3 6h18"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6"/><path d="M8 6V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/>
                                                </svg>
                                            </button>
                            ...
                            @empty
                                <tr>
                                    <td colspan="6" class="kb-empty">Belum ada jadwal. Klik "Tambah Jadwal" untuk menambahkan.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
...
<dialog id="dlgMandor" class="kb kb-dlg" aria-labelledby="judulMandor">
    <div class="kb-dlg__box">
        ...
        <form id="formMandor" method="post" action="{{ url('admin/mandor') }}" data-url-dasar="{{ url('admin/mandor') }}">
            @csrf
            ...
            <div class="kb-dlg__body">
                <div class="kb-form">
                    <div class="kb-field kb-field--full">
                        ...
                        <input class="kb-input" id="fNamaMandor" name="nama" type="text" maxlength="100" placeholder="Contoh: Budi Santoso" value="{{ old('nama') }}" required>
                        @error('nama') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>

                    <div class="kb-field">
                        <label class="kb-label" for="fPhoneMandor">No. HP</label>
                        <input class="kb-input" id="fPhoneMandor" name="phone" type="text" maxlength="20" placeholder="0812-3456-7890" value="{{ old('phone') }}" required>
                        @error('phone') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>

                    <div class="kb-field">
                        <label class="kb-label" for="fBlokMandor">Blok yang Dikelola</label>
                        <select class="kb-select" id="fBlokMandor" name="blok_kelola" required>
                            <option value="">Pilih Blok</option>
                            @foreach ($bloks as $b)
                                <option value="{{ $b->nama_blok }}" data-afdeling="{{ $b->afdeling }}" {{ old('blok_kelola') === $b->nama_blok ? 'selected' : '' }}>
                                    {{ $b->nama_blok }}
                                </option>
                            @endforeach
                        </select>
                        @error('blok_kelola') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>

                    <div class="kb-field">
                        <label class="kb-label" for="fAfdelingMandor">Afdeling</label>
                        <input class="kb-input" id="fAfdelingMandor" name="afdeling" type="text" maxlength="100" placeholder="Terisi otomatis dari blok" value="{{ old('afdeling') }}" required>
                    ...
                    <p class="kb-hint kb-field kb-field--full" id="hintAkunBaru">
                        Akun login akan dibuat otomatis (username sementara &amp; password default). Atur username &amp; password final di tab "Kelola Akun".
                    </p>
                </div>
            </div>

            <div class="kb-dlg__actions">
                <button type="button" class="kb-btn kb-btn--ghost" id="btnTutupMandor" data-aksi="tutup">Batal</button>
        ...
        <div class="kb-dlg__body">
            <div class="kb-form">
                ...
                <div class="kb-field kb-field--full">
                    ...
                    <p class="kb-hint">Password disimpan terenkripsi (hash) sehingga tidak bisa ditampilkan. Gunakan tombol Ubah untuk mengatur ulang.</p>
                </div>
                <div class="kb-field kb-field--full">
...
<dialog id="dlgAkun" class="kb kb-dlg kb-dlg--sm" aria-labelledby="judulAkun">
    <div class="kb-dlg__box">
        ...
        <form id="formAkun" method="post" action="{{ url('admin/mandor') }}" data-url-dasar="{{ url('admin/mandor') }}">
            @csrf
            @method('PUT')
            ...
            <div class="kb-dlg__body">
                <div class="kb-form">
                    ...
                    <div class="kb-field kb-field--full">
                        ...
                        <input class="kb-input" id="fUsername" name="username" type="text" maxlength="50" placeholder="username login" required>
                        @error('username') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>
                    <div class="kb-field">
                        <label class="kb-label" for="fPassword">Password Baru</label>
                        <input class="kb-input" id="fPassword" name="password" type="password" minlength="6" placeholder="Kosongkan jika tidak diubah">
                        @error('password') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>
                    <div class="kb-field">
                        <label class="kb-label" for="fPasswordConfirm">Ulangi Password</label>
                        <input class="kb-input" id="fPasswordConfirm" name="password_confirmation" type="password" minlength="6" placeholder="Ulangi password baru">
                    </div>
                </div>
            </div>
            <div class="kb-dlg__actions">
...
<dialog id="dlgHapusAkun" class="kb kb-dlg kb-dlg--sm" aria-labelledby="judulHapusAkun">
    <div class="kb-dlg__box">
        ...
        <form id="formHapusAkun" method="post" action="{{ url('admin/mandor') }}">
            ...
            <div class="kb-dlg__body">
                <p class="kb-dlg__text">
                    Akun login <strong id="namaHapusAkun"></strong> akan dihapus dan mandor tidak bisa login lagi. Data profil mandor ikut terhapus.
                </p>
            </div>
            <div class="kb-dlg__actions">
                <button type="button" class="kb-btn kb-btn--ghost" data-aksi="tutup">Batal</button>
...
<dialog id="dlgJadwal" class="kb kb-dlg" aria-labelledby="judulJadwal">
    <div class="kb-dlg__box">
        ...
        <form id="formJadwal" method="post" action="{{ url('admin/mandor/jadwal') }}" data-url-dasar="{{ url('admin/mandor/jadwal') }}">
            @csrf
            ...
            <div class="kb-dlg__body">
                <div class="kb-form">
                    <div class="kb-field kb-field--full">
                        ...
                        @error('mandor_id') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>

                    <div class="kb-field kb-field--full">
                        <label class="kb-label" for="fBlokJadwal">Blok / Afdeling</label>
                        <select class="kb-select" id="fBlokJadwal" name="blok_id" required>
                            <option value="">Pilih Blok</option>
                            @foreach ($bloks as $b)
                                <option value="{{ $b->id }}">{{ $b->nama_blok }} &ndash; {{ $b->afdeling }}</option>
                            @endforeach
                        </select>
                        @error('blok_id') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="kb-field">
                        ...
                        @error('tanggal_mulai') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>
                    ...
                    <div class="kb-field">
                        ...
                        @error('tanggal_selesai') <p class="kb-err">{{ $message }}</p> @enderror
                    </div>

                    <div class="kb-field kb-field--full">
                        <label class="kb-label" for="fKeterangan">Keterangan (opsional)</label>
                        <input class="kb-input" id="fKeterangan" name="keterangan" type="text" maxlength="255" placeholder="Catatan tambahan">
                    </div>
                </div>
            </div>
            <div class="kb-dlg__actions">
    ...
    (function () {
        ...
        tabButtons.forEach(function (btn) {
            btn.addEventListener('click', function () {
                tabButtons.forEach(function (b) { b.classList.remove('kb-tab--on'); });
        ...
        var fieldsMandor = ['nama', 'phone', 'blok_kelola', 'afdeling', 'status'];
        var judulModeMandor = { tambah: 'Tambah Mandor', ubah: 'Ubah Mandor', lihat: 'Detail Mandor' };
        ...
        function bukaFormMandor(mode, data, pertahankan) {
            ...
            if (!pertahankan) {
                formMandor.querySelectorAll('.kb-err').forEach(function (el) { el.remove(); });
            ...
            fieldsMandor.forEach(function (f) { formMandor.elements[f].disabled = lihat; });
            btnSimpanMandor.hidden = lihat;
            btnTutupMandor.textContent = lihat ? 'Tutup' : 'Batal';
            dlgMandor.showModal();
        }

        // Isi otomatis afdeling saat blok dipilih
        document.getElementById('fBlokMandor').addEventListener('change', function (e) {
            var opt = e.target.selectedOptions[0];
            document.getElementById('fAfdelingMandor').value = opt ? (opt.dataset.afdeling || '') : '';
        });
        ...
        function bukaAkun(data, pertahankan) {
            ...
            if (!pertahankan) {
                formAkun.querySelectorAll('.kb-err').forEach(function (el) { el.remove(); });
            }
            dlgAkun.showModal();
        }
        ...
        function bukaLihatAkun(d) {
            ...
            document.getElementById('laStatus').value = d.status ? d.status.charAt(0).toUpperCase() + d.status.slice(1) : '-';
        ...
        var fieldsJadwal = ['mandor_id', 'blok_id', 'tanggal_mulai', 'tanggal_selesai', 'keterangan'];
        ...
        function bukaFormJadwal(mode, data, pertahankan) {
            ...
            if (!pertahankan) {
                formJadwal.querySelectorAll('.kb-err').forEach(function (el) { el.remove(); });
        ...
        document.addEventListener('click', function (e) {
            ...
            if (aksi === 'tambah-mandor') { bukaFormMandor('tambah', null, false); return; }
            if (aksi === 'tambah-jadwal') { bukaFormJadwal('tambah', null, false); return; }
        ...
        [dlgMandor, dlgHapusMandor, dlgAkun, dlgLihatAkun, dlgHapusAkun, dlgJadwal, dlgHapusJadwal].forEach(function (d) {
        ...
        @if ($errors->any())
            ...
            panels.forEach(function (p) { p.hidden = p.dataset.tabPanel !== tabError; });
